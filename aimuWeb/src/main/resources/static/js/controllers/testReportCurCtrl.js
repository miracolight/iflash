define(["angular"], function(angular) {
	return function($scope, $modalInstance, patient, test) {
		...
		$scope.last_update_date = patient.lastUpdateDate;
		...
		$scope.init = function() {
			...
			if (data.hasOwnProperty('examCurLeft')) {
				showResult('canvasTextLeft', 'canvasShadeLeft', data.examCurLeft);
			}

			if (data.hasOwnProperty('examCurRight')) {
				showResult('canvasTextRight', 'canvasShadeRight', data.examCurRight);
			}

			if (data.hasOwnProperty('examCurLeft') && data.hasOwnProperty('examCurRight')) {
				$scope.eye='双眼';
			} else if (data.hasOwnProperty('examCurLeft') ){
				$scope.eye='左眼';
			} else if (data.hasOwnProperty('examCurRight')){
				$scope.eye='右眼';
			}
		}
