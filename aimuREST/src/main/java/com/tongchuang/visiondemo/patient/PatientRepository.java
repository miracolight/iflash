...
public interface PatientRepository extends CrudRepository<Patient, String> {
	 ...
	 @Query("SELECT p FROM Patient p where p.status<>'DELETED' and p.name like :filter order by p.lastUpdateDate desc")
	 ...
	 @Query("SELECT p FROM Patient p, Relationship r where r.relationshipType='DOCTOR_PATIENT' "
	 		...
	 		+ "and r.objectId=p.patientId and p.status<>'DELETED' order by p.lastUpdateDate desc")
